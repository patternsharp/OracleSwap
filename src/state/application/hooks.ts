...
export function useDexWarningOpen(): boolean {
  const isShow = useSelector((state: AppState) => state.application.dexUseWarning)
  return isShow
}

export function useToggleDexWarning(): () => void {
  const isShow = useDexWarningOpen()
  const dispatch = useAppDispatch()
  return useCallback(() => dispatch(setDexUseWarning({ dexUseWarning: !isShow })), [dispatch, isShow])
}
